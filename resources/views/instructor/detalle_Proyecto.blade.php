    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start px-4 italic">
            ...
            <x-card class="p-0 overflow-hidden border-none shadow-2xl group rounded-[4rem] bg-white italic" shadow="none">
                        ...
                        <div class="flex flex-wrap items-end justify-between gap-8 italic">
                            <div class="space-y-6 flex-1 min-w-[350px] italic">
                                <div class="flex items-center gap-4 italic font-bold">
                                    <x-badge class="bg-[#FF6B00] text-white border-none py-2 px-6 rounded-xl text-[10px] font-black uppercase italic shadow-2xl shadow-[#FF6B00]/40 italic">
                                ...
                                <p class="text-orange-400 font-black text-sm uppercase tracking-[0.4em] flex items-center gap-4 italic opacity-80 italic">
                    ...
                    <form action="{{ route('instructor.proyectos.imagen', $proyecto->pro_id) }}" method="POST" enctype="multipart/form-data" class="flex flex-col md:flex-row gap-8 italic font-bold">
                        ...
                        <div class="flex-1 relative italic">
                            <input type="file" name="imagen" accept="image/*" required 
                                class="w-full px-8 py-6 rounded-2xl border-4 border-dashed border-white/10 bg-white/5 font-black text-white/40 text-sm focus:border-[#FF6B00]/40 transition-all cursor-pointer hover:border-[#FF6B00]/20 uppercase italic italic">
                        </div>
                        <x-button variant="primary" type="submit" shadow="orange" class="px-12 py-6 text-[10px] font-black uppercase italic shadow-2xl">EJECUTAR ACTUALIZACIÓN</x-button>
                    </form>
                </div>
                ...
                <div class="p-12 md:p-16 space-y-16 italic">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-10 italic font-bold">
                        <div class="p-8 rounded-[2.5rem] bg-slate-50 border-4 border-white shadow-xl space-y-4 group/stat hover:bg-orange-50 transition-all italic">
                            <p class="text-[9px] font-black text-slate-300 uppercase tracking-[0.4em] italic group-hover/stat:text-[#E65100] transition-colors italic leading-none">FECHA DE LANZAMIENTO</p>
                    ...
                    <div class="space-y-8 italic">
                        ...
                        <div class="text-slate-500 leading-relaxed text-lg font-bold uppercase italic tracking-tight opacity-80 whitespace-pre-line italic border-l-8 border-[#FF6B00] pl-10 italic">
            ...
            <x-card class="p-12 md:p-16 border-none shadow-2xl rounded-[4rem] bg-slate-900 relative overflow-hidden italic" shadow="none">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-10 mb-20 relative z-10 italic">
                    <div class="space-y-4 italic">
                        <h3 class="text-4xl md:text-5xl font-black text-white tracking-tighter uppercase italic leading-none italic">Mapa de Ruta <span class="text-[#FF6B00]">Técnica</span></h3>
                    ...
                    <x-button variant="primary" shadow="orange" class="px-10 py-5 rounded-[1.5rem] text-[10px] font-black uppercase italic shadow-2xl hover:scale-105 transition-all active:scale-95 italic" onclick="document.getElementById('stageForm').classList.toggle('hidden')">
                    ...
                    <form action="{{ route('instructor.etapas.crear', $proyecto->pro_id) }}" method="POST" class="space-y-10 italic font-bold">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-10 italic">
                            <div class="space-y-4 italic">
                                ...
                                <input type="number" name="orden" placeholder="IDX" required class="w-full px-8 py-6 rounded-2xl bg-white/5 border-none text-white font-black focus:ring-4 focus:ring-[#FF6B00]/20 italic placeholder:text-white/10 uppercase italic">
                            </div>
                            <div class="md:col-span-3 space-y-4 italic">
                                ...
                                <input type="text" name="nombre" placeholder="EJ: ANÁLISIS DE SISTEMAS" required class="w-full px-8 py-6 rounded-2xl bg-white/5 border-none text-white font-black focus:ring-4 focus:ring-[#FF6B00]/20 italic placeholder:text-white/10 uppercase italic tracking-tighter">
                            </div>
                        </div>
                        <div class="space-y-4 italic">
                            ...
                            <textarea name="descripcion" placeholder="DETALLE LAS EVIDENCIAS REQUERIDAS PARA ESTA ETAPA OPERATIVA..." required class="w-full px-8 py-8 rounded-2xl bg-white/5 border-none text-white font-black focus:ring-4 focus:ring-[#FF6B00]/20 italic placeholder:text-white/10 min-h-[160px] uppercase italic tracking-tight"></textarea>
                        </div>
                        <div class="flex justify-end gap-6 italic">
                            ...
                            <x-button variant="primary" type="submit" shadow="orange" class="py-5 px-12 text-[10px] font-black uppercase italic shadow-2xl italic">PUBLICAR FASE ACADÉMICA</x-button>
                        </div>
                    </form>
                </div>
                ...
                <div class="space-y-10 relative before:absolute before:left-12 before:top-4 before:bottom-4 before:w-1.5 before:bg-white/5 before:rounded-full italic px-2 italic">
                    @forelse($etapas as $index => $etapa)
                        <div class="group relative bg-white pb-10 pt-10 px-12 rounded-[3.5rem] shadow-2xl hover:shadow-[#FF6B00]/10 transition-all duration-700 italic border-4 border-transparent hover:border-[#FF6B00]/10 italic">
                            <div class="absolute -left-[4.25rem] md:-left-[5.25rem] top-10 w-16 h-16 rounded-[1.5rem] {{ $index == 0 ? 'bg-[#FF6B00] text-white shadow-2xl shadow-[#FF6B00]/40' : 'bg-slate-800 text-white shadow-2xl shadow-black/40' }} border-8 border-slate-900 shadow-xl flex items-center justify-center font-black text-2xl z-20 transition-all duration-700 group-hover:scale-110 group-hover:rotate-12 italic">
                            ...
                            <div class="flex justify-between items-start gap-10 italic">
                                <div class="space-y-6 flex-1 italic">
                                    <h4 class="text-3xl font-black text-slate-900 group-hover:text-[#FF6B00] transition-colors uppercase italic tracking-tighter italic leading-none">{{ $etapa->eta_nombre }}</h4>
        ...
        <div class="space-y-12 lg:sticky lg:top-12 font-bold italic">
            ...
            <x-card class="p-12 text-center border-none shadow-2xl rounded-[3.5rem] overflow-hidden relative bg-white italic" shadow="none">
                <div class="absolute top-0 right-0 w-32 h-32 bg-[#FF6B00]/5 rounded-bl-[120px] italic"></div>
                ...
                <div class="inline-flex items-center gap-4 px-10 py-5 rounded-[2rem] bg-orange-50 text-[#E65100] font-black text-[10px] uppercase tracking-[0.2em] border-2 border-orange-100 mb-12 shadow-xl italic shadow-[#FF6B00]/5 italic">
                    <span class="w-3 h-3 rounded-full bg-[#FF6B00] animate-ping italic"></span>
                ...
                <div class="grid grid-cols-2 gap-10 border-t border-slate-50 pt-12 italic">
                    ...
                    <div class="space-y-3 border-l border-slate-50 italic">
                        <p class="text-5xl font-black text-[#FF6B00] italic tracking-tighter leading-none italic">{{ str_pad($etapas->count(), 2, '0', STR_PAD_LEFT) }}</p>
            ...
            <div class="grid grid-cols-1 gap-6 italic font-bold">
                <x-button href="{{ route('instructor.reporte', $proyecto->pro_id) }}" variant="primary" shadow="orange" class="w-full justify-center py-7 rounded-[2.5rem] group shadow-2xl shadow-[#FF6B00]/10 italic text-[11px] uppercase tracking-widest italic">
            ...
            <x-card class="p-0 border-none shadow-2xl rounded-[3.5rem] overflow-hidden bg-white italic font-bold" shadow="none">
                ...
                <div class="p-10 space-y-6 max-h-[550px] overflow-y-auto custom-scrollbar italic">
                    @forelse($postulaciones->where('pos_estado', 'Pendiente') as $p)
                        <div class="p-8 rounded-[3rem] bg-white border-4 border-slate-50 shadow-xl space-y-8 hover:shadow-[#FF6B00]/5 hover:border-[#FF6B00]/10 transition-all group italic font-bold">
                            <div class="flex items-center gap-6 italic">
                                <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#FF6B00] to-teal-600 text-white flex items-center justify-center font-black text-2xl shadow-2xl shadow-[#FF6B00]/20 group-hover:rotate-12 transition-transform italic border-4 border-white italic">
                                ...
                                <x-confirm-modal 
                                    ...
                                    <form action="{{ route('instructor.postulaciones.estado', $p->pos_id) }}" method="POST" class="italic">
                                        ...
                                        <button type="button" @click="$dispatch('open-confirm-modal-modal-alta-{{ $p->pos_id }}')" 
                                            class="w-full py-4 rounded-2xl bg-orange-50 text-orange-700 font-black text-[10px] hover:bg-[#E65100] hover:text-white transition-all uppercase tracking-widest italic shadow-sm active:scale-95">ALTA</button>
                                    </form>
                                </x-confirm-modal>
            ...
            <x-card class="p-0 border-none shadow-2xl rounded-[3.5rem] overflow-hidden bg-white italic font-bold" shadow="none">
                ...
                <div class="p-10 space-y-5 italic">
                    @forelse($integrantes as $i)
                        <div class="flex items-center gap-6 p-6 rounded-[2.5rem] bg-slate-50 hover:bg-orange-50 border-4 border-white hover:border-[#FF6B00]/10 transition-all group shadow-xl italic font-bold">
                            <div class="w-14 h-14 rounded-2xl bg-white shadow-2xl flex items-center justify-center border-4 border-[#FF6B00]/10 italic group-hover:scale-110 group-hover:rotate-6 transition-transform">
                                <span class="text-lg font-black text-[#E65100] italic">{{ substr($i->aprendiz->apr_nombre, 0, 1) }}</span>
                            </div>
                            <div class="min-w-0 italic">
