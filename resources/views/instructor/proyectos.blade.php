...
<div class="max-w-7xl mx-auto space-y-12 pb-24 italic font-bold">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-10 px-4 italic font-bold">
        <div class="space-y-6 italic">
            ...
            <h2 class="text-5xl md:text-6xl font-black text-slate-900 tracking-tighter uppercase italic leading-none italic">
                MIS <span class="text-[#FF6B00] block mt-2 text-6xl md:text-7xl italic">PROYECTOS</span>
            </h2>
        </div>
        ...
        <div class="flex items-center gap-6 italic">
            <div class="bg-slate-900 px-8 py-5 rounded-[2rem] shadow-2xl flex items-center gap-5 border-2 border-white/5 italic">
                <span class="text-4xl font-black text-orange-400 tracking-tighter leading-none italic">{{ str_pad($proyectos->count(), 2, '0', STR_PAD_LEFT) }}</span>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 px-4 italic">
        @forelse($proyectos as $p)
            <x-card class="group border-none shadow-2xl hover:shadow-[#FF6B00]/10 transition-all duration-700 p-0 overflow-hidden bg-white flex flex-col h-full rounded-[4rem] italic" shadow="none">
                ...
                <div class="relative h-64 md:h-72 overflow-hidden italic">
                    ...
                    <div class="absolute top-8 left-8 italic">
                        <x-badge class="bg-[#FF6B00]/90 text-white backdrop-blur-xl border-none px-4 py-2 rounded-xl text-[10px] font-black uppercase shadow-2xl italic tracking-widest leading-none">
                    ...
                    <div class="absolute bottom-8 left-8 right-8 italic">
                        <div class="flex items-center gap-3 text-white/50 text-[9px] font-black uppercase tracking-[0.3em] mb-2 italic">
                            <i class="fas fa-building text-orange-400 italic"></i>
                ...
                <div class="p-10 flex-1 flex flex-col space-y-8 italic font-bold">
                    <div class="flex flex-wrap gap-4 italic">
                        <x-badge class="bg-slate-50 text-slate-400 border-2 border-white px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest italic shadow-sm italic leading-none">
                            <i class="fas fa-tag mr-2 text-[#FF6B00] italic"></i> {{ strtoupper($p->pro_categoria ?? 'TÉCNICO') }}
                        </x-badge>
                        <x-badge class="bg-slate-50 text-slate-400 border-2 border-white px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest italic shadow-sm italic leading-none">
                            <i class="fas fa-calendar-alt mr-2 text-[#FF6B00] italic"></i> {{ \Carbon\Carbon::parse($p->pro_fecha_publi)->translatedFormat('d M, Y') }}
                        </x-badge>
                    </div>

                    <div class="grid grid-cols-2 gap-6 mt-auto italic">
                        <div class="bg-slate-900 p-6 rounded-[2rem] border-4 border-slate-50 shadow-2xl italic group-hover:bg-[#E65100] transition-colors duration-700">
                            <span class="block text-3xl font-black text-orange-400 group-hover:text-white italic tracking-tighter leading-none italic">{{ str_pad($p->total_aprendices ?? $p->postulaciones->count(), 2, '0', STR_PAD_LEFT) }}</span>
                            <span class="text-[8px] text-white/30 font-black uppercase tracking-[0.3em] mt-2 block italic leading-none">TALENTOS</span>
                        </div>
                        <x-button href="{{ route('instructor.proyecto.detalle', $p->pro_id) }}" variant="primary" shadow="orange" class="py-6 rounded-[2rem] text-[10px] font-black uppercase tracking-[0.3em] shadow-2xl hover:scale-105 active:scale-95 transition-all italic leading-none flex items-center justify-center">
