...
<div class="max-w-6xl mx-auto space-y-12 pb-24 italic font-bold">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-10 px-4 italic">
        <div class="space-y-8 italic">
            ...
            <div class="space-y-4 italic">
                ...
                <h2 class="text-5xl md:text-6xl font-black text-slate-900 tracking-tighter uppercase italic leading-[0.9] italic">
                    SENTENCIA DE <br> <span class="text-emerald-500 text-6xl md:text-8xl block mt-2">EVIDENCIAS</span>
                </h2>
        ...
        <div class="flex items-center gap-6 italic">
            <div class="px-10 py-6 rounded-[2rem] bg-slate-900 text-emerald-400 border-none text-[11px] font-black shadow-2xl flex items-center gap-5 uppercase italic tracking-widest animate-in fade-in slide-in-from-right duration-700 italic border-2 border-white/5">
    ...
    @if(session('success'))
        <div class="mx-4 p-10 rounded-[3.5rem] bg-emerald-50 border-4 border-white flex items-center gap-8 animate-in zoom-in-95 duration-700 italic shadow-2xl shadow-emerald-500/10 italic">
            <div class="w-20 h-20 rounded-[1.5rem] bg-emerald-500 text-white flex items-center justify-center shadow-2xl shadow-emerald-500/30 rotate-6 italic group-hover:rotate-0 transition-transform italic border-4 border-white italic">
                <i class="fas fa-check-double text-3xl italic font-bold"></i>
            </div>
            <div class="italic">
                <p class="font-black text-2xl text-emerald-900 uppercase italic tracking-tighter italic leading-none">Protocolo Validado</p>
                <p class="text-emerald-600 font-black text-[10px] uppercase italic tracking-[0.3em] mt-3 italic opacity-60 italic">{{ session('success') }}</p>
            </div>
        </div>
    @endif
    ...
    <div class="space-y-12 px-4 italic font-bold">
        @forelse($evidencias as $evidencia)
            <x-card class="p-0 overflow-hidden border-none shadow-2xl rounded-[4rem] bg-white group hover:shadow-emerald-500/5 transition-all duration-700 italic" shadow="none">
                ...
                <div class="p-10 md:p-12 bg-slate-50 border-b-4 border-white flex flex-wrap justify-between items-center gap-10 italic">
                    <div class="flex items-center gap-10 italic">
                        <div class="relative italic">
                            <div class="w-20 h-20 rounded-[1.5rem] bg-white shadow-2xl flex items-center justify-center text-emerald-600 group-hover:rotate-12 transition-transform italic border-4 border-slate-50 italic">
                        ...
                        <div class="min-w-0 italic">
                            <h4 class="text-3xl font-black text-slate-900 group-hover:text-emerald-600 transition-colors uppercase italic tracking-tighter leading-none mb-3 italic">HITO: {{ $evidencia->eta_nombre }}</h4>
                            <div class="flex items-center gap-4 text-slate-400 text-[10px] font-black uppercase tracking-[0.3em] italic opacity-80 italic">
                                <i class="fas fa-user-astronaut text-emerald-500 italic font-bold"></i>
                        ...
                        <div class="lg:col-span-3 space-y-12 italic">
                            <form action="{{ route('instructor.evidencias.calificar', $evidencia->evid_id) }}" method="POST" class="space-y-12 italic">
                                ...
                                <div class="space-y-8 italic">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 italic">
                                        <label class="relative group cursor-pointer italic">
                                            ...
                                            <div class="p-8 rounded-[2.5rem] border-4 border-slate-50 bg-slate-50 flex flex-col items-center gap-4 group-hover:bg-emerald-50 peer-checked:border-emerald-500 peer-checked:bg-emerald-500 peer-checked:text-white transition-all shadow-xl active:scale-95 italic text-slate-400 italic">
                                ...
                                <div class="space-y-6 italic">
                                    ...
                                    <textarea name="comentario" class="w-full min-h-[200px] p-10 rounded-[3rem] bg-slate-50 border-none focus:ring-8 focus:ring-emerald-500/10 focus:bg-white focus:outline-none font-black text-sm text-slate-700 transition-all shadow-inner uppercase italic placeholder:text-slate-200 tracking-tight leading-relaxed italic" placeholder="ESPECIFIQUE HALLAZGOS TÉCNICOS Y REQUERIMIENTOS ESTRATÉGICOS PARA EL OPERADOR...">{{ $evidencia->evid_comentario }}</textarea>
                                </div>

                                <x-button variant="primary" size="lg" shadow="emerald" class="w-full justify-center py-7 rounded-[2.5rem] text-[11px] font-black uppercase italic shadow-2xl shadow-emerald-500/10 active:scale-95 transition-transform italic tracking-widest italic leading-none">
                        ...
                        <div class="lg:col-span-2 space-y-10 italic">
                            <div class="p-10 rounded-[3.5rem] bg-slate-900 border-none space-y-10 italic shadow-2xl shadow-black/40 text-white relative overflow-hidden italic">
                                ...
                                <h5 class="text-[10px] font-black text-white/40 uppercase tracking-[0.3em] border-b-2 border-white/5 pb-6 flex items-center gap-4 italic opacity-80 italic leading-none">
                                    <i class="fas fa-satellite-dish text-emerald-500 italic text-xl font-bold"></i> EXPEDIENTE TÉCNICO
                                </h5>
                                
                                <div class="space-y-6 italic">
                                    @if($evidencia->evid_archivo)
                                        <a href="{{ asset('storage/' . $evidencia->evid_archivo) }}" target="_blank" 
                                            class="flex items-center gap-6 p-8 bg-white/5 rounded-[2.5rem] border-4 border-white/5 hover:bg-white/10 hover:border-emerald-500/40 hover:shadow-2xl hover:shadow-emerald-500/10 transition-all group italic italic">
                                            <div class="w-16 h-16 rounded-2xl bg-emerald-500 text-white flex items-center justify-center text-3xl group-hover:scale-110 group-hover:rotate-12 transition-transform italic shadow-2xl shadow-emerald-500/40 italic border-4 border-white/10 italic">
                                                <i class="fas fa-file-pdf italic font-bold"></i>
                                            </div>
                                            <div class="min-w-0 italic">
                                                <p class="text-[9px] font-black text-emerald-400 uppercase tracking-[0.4em] mb-2 italic leading-none italic">DESCARGAR PROTOCOLO</p>
                                ...
                                <div class="space-y-6 pt-10 border-t-2 border-white/5 italic font-bold">
                                    <div class="flex justify-between items-center text-[9px] font-black uppercase tracking-[0.4em] text-white/30 italic">
                                        ...
                                        <x-badge class="bg-emerald-500 text-white border-none py-1.5 px-4 rounded-lg text-[8px] font-black italic shadow-xl shadow-emerald-500/20 italic">NOMINAL OK</x-badge>
                                    </div>
                                    <div class="h-3 w-full bg-white/5 rounded-full overflow-hidden italic shadow-inner border-2 border-white/5">
                                        <div class="h-full bg-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.5)] transition-all duration-1000 italic" style="width: 100%"></div>
                                    </div>
        ...
        @empty
            <div class="py-48 text-center italic border-8 border-dashed border-slate-50 rounded-[5rem] group hover:border-emerald-500/10 transition-all duration-1000 bg-slate-50/30 italic">
                <div class="relative inline-block italic">
                    <div class="absolute inset-0 bg-emerald-500/10 rounded-full blur-[120px] animate-pulse italic"></div>
                    <div class="relative mb-12 inline-flex p-12 rounded-[4rem] bg-white text-emerald-100 shadow-2xl ring-[30px] ring-white group-hover:scale-110 group-hover:rotate-12 transition-all duration-1000 italic">
                        <i class="fas fa-user-astronaut text-9xl text-emerald-500 italic drop-shadow-2xl font-bold"></i>
                    </div>
                </div>
