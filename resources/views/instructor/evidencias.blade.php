    ...
    <div style="margin-bottom: 32px; display: flex; justify-content: space-between; align-items: flex-end;">
        <div>
            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
                <a href="{{ route('instructor.proyecto.detalle', $proyecto->pro_id) }}" style="color: var(--primary); text-decoration: none; font-size: 0.9rem; font-weight: 700;">
            ...
            <p style="color:var(--text-light); font-size:15px; margin-top:4px; font-weight: 500;">Proyecto: <span style="color: var(--text); font-weight: 800;">{{ $proyecto->pro_titulo_proyecto }}</span></p>
        </div>
            ...
            <div class="glass-card" style="padding: 0; overflow: hidden; border-radius: 20px;">
                ...
                <div style="padding: 1.5rem 2rem; background: #f8fafc; border-bottom: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center;">
                    <div style="display: flex; align-items: center; gap: 15px;">
                        ...
                        <div>
                            <h4 style="font-size: 1.15rem; font-weight: 800; color: var(--text);">Etapa: {{ $evidencia->eta_nombre }}</h4>
                            <p style="font-size: 0.85rem; color: var(--text-light); font-weight: 600;"><i class="fas fa-user-graduate" style="margin-right: 6px;"></i>{{ $evidencia->apr_nombre }} {{ $evidencia->apr_apellido }}</p>
                        </div>
                    </div>
                    ...
                    <div style="text-align: right;">
                        ...
                        <p style="font-size: 0.95rem; font-weight: 700; color: var(--text);">{{ \Carbon\Carbon::parse($evidencia->evid_fecha)->format('d/m/Y - h:m A') }}</p>
                    </div>
                </div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 300px; gap: 2.5rem;">
                        ...
                        <form action="{{ route('instructor.evidencias.calificar', $evidencia->evid_id) }}" method="POST">
                            ...
                            <div style="margin-bottom: 2rem;">
                                ...
                                <textarea name="comentario" class="instructor-input-control" style="min-height: 120px; padding: 1.25rem;" placeholder="Escribe aquí los comentarios sobre la calidad del entregable...">{{ $evidencia->evid_comentario }}</textarea>
                            </div>
                        ...
                        <div class="instructor-deliverable-info">
                            <div>
                                ...
                                @if($evidencia->evid_archivo)
                                    <a href="{{ asset('storage/' . $evidencia->evid_archivo) }}" target="_blank" class="instructor-team-member" style="text-decoration: none; padding: 12px;">
                                        ...
                                        <div style="overflow: hidden;">
                                            ...
                                            <p style="font-size: 0.7rem; color: var(--text-lighter); font-weight: 600;">Formato: {{ strtoupper(pathinfo($evidencia->evid_archivo, PATHINFO_EXTENSION)) }}</p>
                                        </div>
                                    </a>
                                @else
