...
<div class="max-w-7xl mx-auto space-y-10 pb-16">
    ...
    <div class="space-y-4">
        <h2 class="text-4xl font-black text-slate-900 tracking-tighter uppercase italic leading-none">
            Bitácora de <span class="text-emerald-500 underline decoration-emerald-500/20 decoration-8 underline-offset-8">Operaciones</span>
        </h2>
    ...
    @if($proyectos->count() > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 font-bold text-slate-900">
            @foreach($proyectos as $proyecto)
                <x-card class="p-8 md:p-10 border-none ring-1 ring-slate-100 shadow-xl hover:shadow-2xl hover:ring-emerald-500/30 transition-all group relative overflow-hidden">
                    <div class="absolute -right-10 -top-10 w-32 h-32 bg-slate-50 rounded-full blur-3xl opacity-50 transition-colors group-hover:bg-emerald-50"></div>
                    
                    <div class="relative space-y-8">
                        <div class="flex justify-between items-start gap-4">
                            <div class="space-y-2 flex-1">
                                <h5 class="text-2xl font-black text-slate-900 tracking-tight uppercase italic group-hover:text-emerald-600 transition-colors leading-tight">{{ $proyecto->pro_titulo_proyecto }}</h5>
                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest italic flex items-center gap-2">
                                    <i class="fas fa-building text-emerald-500"></i> {{ $proyecto->emp_nombre }}
                                </p>
                            </div>
                            <div class="flex flex-col items-end gap-2">
                                @switch($proyecto->pos_estado)
                                    @case('Aprobada')
                                        <x-badge class="bg-emerald-500 text-white border-none py-1.5 px-4 rounded-xl text-[9px] font-black uppercase shadow-lg shadow-emerald-100">ADMITIDO</x-badge>
                                        @break
                                    @case('Rechazada')
                        ...
                        <div class="bg-slate-50 p-6 rounded-[2rem] flex items-center justify-between group/status font-bold">
                            ...
                            <div class="space-y-1">
                                ...
                                <h4 class="text-sm font-black text-slate-900 uppercase italic group-hover/status:text-emerald-600 transition-colors">{{ $proyecto->pro_estado }}</h4>
                            </div>
                            <div class="text-right">
                                ...
                                @else
                                    <span class="text-[10px] font-black text-emerald-500 uppercase italic flex items-center gap-2">
                        ...
                        @if($proyecto->pos_estado === 'Aprobada')
                            <x-button :href="route('aprendiz.proyecto.detalle', $proyecto->pro_id)" variant="primary" shadow="emerald" class="w-full py-5 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] flex items-center justify-center gap-3 active:scale-95 transition-all group/btn">
