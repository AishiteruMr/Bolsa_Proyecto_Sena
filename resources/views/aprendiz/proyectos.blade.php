...
<div class="max-w-7xl mx-auto space-y-12 pb-24 italic font-bold">
    ...
    <x-card class="bg-slate-900 border-none rounded-[4rem] p-12 md:p-24 text-center relative overflow-hidden shadow-2xl group text-white italic" shadow="none">
        ...
        <div class="absolute -right-20 -top-20 w-[30rem] h-[30rem] bg-[#FF6B00]/10 rounded-full blur-[120px] group-hover:bg-[#FF6B00]/20 transition-all duration-[2000ms] italic"></div>
        ...
        <div class="relative space-y-12 italic">
            <div class="space-y-6 italic">
                <x-badge class="bg-[#FF6B00] text-white border-none px-6 py-2 rounded-full font-black tracking-[0.3em] text-[10px] shadow-2xl shadow-[#FF6B00]/40 uppercase italic italic">
                ...
                <h2 class="text-5xl md:text-7xl font-black text-white tracking-tighter leading-none uppercase italic italic">
                    FORJA TU DESTINO <br>
                    <span class="text-[#FF6B00] text-6xl md:text-8xl block mt-2">PROFESIONAL</span>
                </h2>
            ...
            <form action="{{ route('aprendiz.proyectos') }}" method="GET" class="max-w-4xl mx-auto relative group/search italic">
                <div class="relative flex items-center bg-white p-3 rounded-[3rem] shadow-2xl ring-4 ring-white/5 focus-within:ring-[#FF6B00]/20 transition-all italic">
                    ...
                    <x-button type="submit" variant="primary" shadow="orange" class="px-12 py-6 rounded-[2.5rem] font-black text-xs uppercase tracking-[0.2em] transition-all hover:scale-105 active:scale-95 italic italic shadow-2xl">
                        RASTREAR
                    </x-button>
                </div>
            </form>
    ...
    <div class="space-y-10 px-4 italic">
        ...
        <div class="flex flex-wrap gap-6 italic">
            <a href="{{ route('aprendiz.proyectos', array_merge(request()->all(), ['categoria' => ''])) }}" 
               class="px-10 py-5 rounded-[1.5rem] font-black text-[10px] tracking-widest transition-all border-4 uppercase italic {{ !request('categoria') ? 'bg-slate-900 text-white border-slate-900 shadow-2xl' : 'bg-white text-slate-400 border-slate-50 hover:border-[#FF6B00] hover:text-[#FF6B00] shadow-xl' }}">
                TODOS LOS HITOS
            </a>
            @foreach($categorias as $cat)
                <a href="{{ route('aprendiz.proyectos', array_merge(request()->all(), ['categoria' => $cat])) }}" 
                   class="px-10 py-5 rounded-[1.5rem] font-black text-[10px] tracking-widest transition-all border-4 uppercase italic {{ request('categoria') == $cat ? 'bg-[#FF6B00] text-white border-[#FF6B00] shadow-2xl shadow-[#FF6B00]/40' : 'bg-white text-slate-400 border-slate-50 hover:border-[#FF6B00] hover:text-[#FF6B00] shadow-xl' }}">
                    {{ strtoupper($cat) }}
                </a>
            @endforeach
        </div>
    ...
    <div class="flex items-center gap-6 px-4 italic">
        <div class="flex items-center gap-4 bg-slate-900 px-8 py-4 rounded-[1.5rem] shadow-2xl italic">
            ...
            <span class="text-[9px] font-black text-orange-400 uppercase tracking-[0.3em] italic leading-tight italic">CONTRATOS <br>RADICADOS</span>
        </div>
        @if(request('buscar'))
            <x-badge class="bg-orange-50 text-[#E65100] border-4 border-white px-6 py-4 rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest italic shadow-xl italic">
    ...
    @if($proyectos->isEmpty())
        <div class="py-32 bg-white rounded-[4rem] border-8 border-slate-50 shadow-inner text-center space-y-12 px-12 italic">
            ...
            <x-button :href="route('aprendiz.proyectos')" variant="secondary" class="bg-slate-900 text-white border-none rounded-[2rem] px-16 py-6 font-black text-[11px] uppercase tracking-widest hover:bg-[#E65100] transition-all italic italic shadow-2xl active:scale-95">
                REINICIAR ESCÁNER
            </x-button>
        </div>
    @else
                ...
                <x-card class="group flex flex-col border-none shadow-xl hover:shadow-[0_40px_80px_-20px_rgba(16,185,129,0.15)] hover:-translate-y-4 transition-all duration-700 rounded-[3.5rem] overflow-hidden bg-white italic" shadow="none">
                    ...
                    <div class="p-10 flex flex-1 flex-col space-y-8 italic">
                        <div class="space-y-4 italic">
                            ...
                            <h3 class="text-2xl font-black text-slate-900 leading-[1.1] uppercase italic tracking-tighter group-hover:text-[#E65100] transition-colors italic h-20 overflow-hidden border-l-8 border-[#FF6B00] pl-6 italic">
                        ...
                        <div class="pt-6 mt-auto italic">
                            @if(in_array($p->pro_id, $postulados))
                                <div class="w-full bg-[#E65100] text-white border-none py-6 rounded-2xl text-center font-black text-[10px] uppercase tracking-[0.3em] italic flex items-center justify-center gap-4 shadow-2xl shadow-[#FF6B00]/20 italic">
                            ...
                            @else
                                <form action="{{ route('aprendiz.postular', $p->pro_id) }}" method="POST" class="italic">
                                    @csrf
                                    <x-button type="submit" variant="primary" shadow="orange" class="w-full py-6 rounded-[1.5rem] font-black text-[10px] uppercase tracking-[0.2em] flex items-center justify-center gap-4 group/btn italic italic shadow-2xl active:scale-95 transition-all">
                                        SOLICITAR ACCESO 
