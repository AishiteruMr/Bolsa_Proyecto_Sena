        ...
        @foreach($proyectos as $proyecto)
            <div class="card mb-4" style="border-left: 4px solid #005a87; box-shadow: 0 2px 8px rgba(0,90,135,0.1);">
                <div class="card-body">
                    ...
                    <h6 style="color: #005a87; font-weight: 600; margin-bottom: 15px;">✓ Evidencias ({{ $evidencias_proyecto->count() }})</h6>
                            ...
                            @foreach($evidencias_proyecto as $evidencia)
                                <div style="background: #f8f9fa; padding: 12px; border-radius: 5px; border-left: 3px solid #6c757d;">
                                    <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
                                        <strong style="color: #005a87;">{{ $evidencia->etapa->eta_nombre ?? 'Etapa sin nombre' }}</strong>
                                        <span class="badge" style="@if($evidencia->evid_estado === 'Aprobada')background-color: #28a745;@elseif($evidencia->evid_estado === 'Rechazada')background-color: #dc3545;@else background-color: #ffc107;@endif color: white;">
                                            {{ $evidencia->evid_estado }}
                                        </span>
                                    </div>
                                    <small style="color: #666; display: block; margin-bottom: 8px;">
                                        <strong>Etapa {{ $evidencia->etapa->eta_orden ?? 'N/A' }}</strong> - 
                                    ...
                                    @if($evidencia->hasFile())
                                        <a href="{{ $evidencia->getFileUrl() }}" target="_blank" class="btn btn-sm" style="background-color: #6c757d; color: white; text-decoration: none; border-radius: 4px; padding: 4px 8px;">
                                            📎 Ver Archivo
                                        </a>
                                    @endif
                                </div>
        ...
        <div class="row mt-4">
            ...
            <div class="col-md-4 mb-4">
                <div class="card text-center" style="background: linear-gradient(135deg, #ffc107 0%, #e0a800 100%); color: white; border: none;">
                    <div class="card-body">
                        <h3 style="margin: 0; font-weight: 600;">{{ $evidencias->where('evid_estado', 'Pendiente')->count() }}</h3>
                        <small>Evidencias Pendientes</small>
                    </div>
                </div>
            </div>
        </div>
