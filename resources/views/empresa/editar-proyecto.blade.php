...
<div class="max-w-5xl mx-auto space-y-12 pb-24 font-bold italic">
    ...
    <div class="space-y-8 px-4">
        <a href="{{ route('empresa.proyectos') }}" class="inline-flex items-center text-[10px] font-black text-emerald-600 uppercase tracking-[0.2em] hover:text-emerald-700 transition-all group italic font-bold">
        ...
        <div class="space-y-4 italic">
            <h2 class="text-5xl md:text-6xl font-black text-slate-900 tracking-tighter uppercase italic leading-none">
                Refinar <span class="text-emerald-500 underline decoration-emerald-500/10 decoration-8 underline-offset-8">Convocatoria</span>
            </h2>
        ...
        <x-card class="p-10 md:p-16 border-none shadow-2xl rounded-[4rem] bg-white space-y-16 italic" shadow="none">
            ...
            <section class="space-y-10 italic">
                <div class="flex items-center gap-6 border-b border-slate-100 pb-8 italic">
                    <div class="w-14 h-14 rounded-2xl bg-emerald-500 text-white flex items-center justify-center shadow-2xl shadow-emerald-500/20 italic rotate-3">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-10 italic">
                    <div class="md:col-span-2 space-y-3 italic">
                        ...
                        <input type="text" name="titulo" value="{{ old('titulo', $proyecto->pro_titulo_proyecto) }}" required maxlength="200"
                            class="w-full px-8 py-5 rounded-[1.5rem] bg-slate-50 border-2 {{ $errors->has('titulo') ? 'border-red-500 bg-red-50' : 'border-slate-100' }} focus:border-emerald-500 focus:bg-white focus:ring-8 focus:ring-emerald-500/5 transition-all font-black text-slate-900 uppercase italic placeholder:text-slate-300 italic" 
                ...
                <div class="space-y-4 italic">
                    ...
                    <div class="flex flex-col md:flex-row gap-6 italic">
                        <div class="relative flex-1 italic">
                            <i class="fas fa-map-marker-alt absolute left-8 top-1/2 -translate-y-1/2 text-emerald-500 pointer-events-none italic"></i>
                            <input type="text" id="ubicacion" name="ubicacion" value="{{ old('ubicacion', $proyecto->pro_ubicacion) }}" required 
                                class="w-full pl-16 pr-8 py-5 rounded-[1.5rem] bg-slate-50 border-2 border-slate-100 focus:border-emerald-500 focus:bg-white transition-all font-black text-slate-900 uppercase italic italic">
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10 italic">
                    <div class="space-y-4 italic">
                        ...
                        <textarea name="requisitos" required rows="4" maxlength="200"
                            class="w-full px-8 py-6 rounded-[2rem] bg-slate-50 border-2 {{ $errors->has('requisitos') ? 'border-red-500 bg-red-50' : 'border-slate-100' }} focus:border-emerald-500 focus:bg-white transition-all font-black text-slate-900 uppercase italic italic">{{ old('requisitos', $proyecto->pro_requisitos_especificos) }}</textarea>
                        @error('requisitos')
                            <p class="text-[9px] font-black text-red-500 uppercase tracking-widest mt-2 ml-4 italic animate-pulse">!! {{ $message }}</p>
                        @enderror
                    </div>
            ...
            <section class="space-y-10 italic">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 p-10 bg-slate-900 rounded-[3.5rem] border-none shadow-2xl relative overflow-hidden italic">
                    <div class="absolute -right-20 -top-20 w-64 h-64 bg-emerald-500/5 rounded-full italic"></div>
                ...
                <div class="flex flex-col md:flex-row gap-12 items-center italic">
                    @if($proyecto->pro_imagen_url)
                        <div class="relative group shrink-0 italic">
                            ...
                            <div class="absolute inset-0 bg-emerald-500/20 opacity-0 group-hover:opacity-100 transition-all rounded-[3rem] flex items-center justify-center backdrop-blur-sm italic">
                                <span class="bg-white text-emerald-600 px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl italic">HOLOGRAMA ACTUAL</span>
                            </div>
                        </div>
                    @endif
                    <div id="image-upload-container" class="flex-1 relative group min-h-[256px] w-full bg-slate-50 border-8 border-slate-50 shadow-2xl ring-1 ring-slate-200 rounded-[3rem] overflow-hidden flex flex-col items-center justify-center transition-all hover:bg-emerald-50/20 italic">
                        <div id="image-upload-content" class="text-center p-12 pointer-events-none transition-all group-hover:scale-110 italic">
                            <div class="w-16 h-16 rounded-2xl bg-white shadow-xl flex items-center justify-center mx-auto mb-4 text-emerald-500 italic">
