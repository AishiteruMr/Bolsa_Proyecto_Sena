    ...
    <div style="display:grid; grid-template-columns:2fr 1fr; gap:28px; align-items:start;">
        ...
        <div style="display:grid; gap:20px; align-content:start;">
            ...
            <div style="padding:28px; background:linear-gradient(135deg, #0f172a, #1e293b); border:none; color:white; position:relative; overflow:hidden; border-radius:var(--radius); box-shadow:var(--shadow); box-sizing:border-box;">
                ...
                <h4 style="font-size:17px; font-weight:900; margin-bottom:8px; color:white;">Socio Verificado</h4>
            ...
            <div class="glass-card" style="padding:24px;">
                ...
                <div style="display:flex; align-items:center; gap:10px; padding:12px 14px; background:{{ $empresa->consentimiento_datos ? 'rgba(62,180,137,0.1)' : '#fef2f2' }}; border-radius:12px; border:1px solid {{ $empresa->consentimiento_datos ? 'rgba(62,180,137,0.2)' : '#fecaca' }}; margin-bottom:12px;">
                    <i class="fas {{ $empresa->consentimiento_datos ? 'fa-check-circle' : 'fa-times-circle' }}" style="color:{{ $empresa->consentimiento_datos ? '#22c55e' : '#ef4444' }}; font-size:18px;"></i>
                    <div>
                        <span style="font-size:13px; font-weight:700; color:{{ $empresa->consentimiento_datos ? '#166534' : '#991b1b' }};">
                            {{ $empresa->consentimiento_datos ? 'Consentimiento otorgado' : 'Consentimiento retirado' }}
                        </span>
                        @if($empresa->fecha_consentimiento)
                            <span style="font-size:11px; color:#64748b; display:block; margin-top:2px;">
                                {{ $empresa->consentimiento_datos ? 'Otorgado' : 'Actualizado' }}: {{ \Carbon\Carbon::parse($empresa->fecha_consentimiento)->format('d/m/Y H:i') }}
                            </span>
                        @endif
                    </div>
                </div>
                <p style="font-size:11px; color:#94a3b8; line-height:1.5; margin-bottom:12px;">
                    Los datos de tu empresa son tratados conforme a la Ley 1581 de 2012.
                </p>
                <div style="display:flex; gap:8px; flex-wrap:wrap;">
                    ...
                    @if($empresa->consentimiento_datos)
                        <form action="{{ route('consentimiento.retirar') }}" method="POST" onsubmit="return confirm('¿Estás seguro de retirar el consentimiento? Esto puede limitar algunas funcionalidades de la plataforma.')" style="display:inline;">
                            @csrf
