...
@section('sidebar-nav')
    ...
    <a href="{{ route('empresa.proyectos') }}" class="nav-item group {{ request()->routeIs('empresa.proyectos*') ? 'active' : '' }}">
        <i class="fas fa-project-diagram group-hover:rotate-12 transition-transform italic text-emerald-500"></i> 
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-10 px-4">
        <div class="space-y-6">
            <div class="flex items-center gap-5 italic font-bold">
                <x-badge class="bg-emerald-500 text-white border-none px-5 py-2 rounded-2xl text-[9px] font-black uppercase tracking-[0.2em] shadow-xl shadow-emerald-500/20 italic">ARCHIVO CORPORATIVO</x-badge>
                <div class="flex items-center gap-2">
                    <span class="w-3 h-3 rounded-full bg-emerald-500 animate-pulse shadow-lg shadow-emerald-500/20"></span>
            ...
            <h2 class="text-5xl md:text-6xl font-black text-slate-900 tracking-tighter uppercase italic leading-none">
                MIS <span class="text-emerald-500">PROYECTOS</span>
            </h2>
        ...
        <x-button variant="primary" :href="route('empresa.proyectos.crear')" shadow="emerald" class="py-6 px-10 rounded-[2rem] group flex items-center gap-4 text-[10px] font-black uppercase italic shadow-2xl active:scale-95 transition-all text-white">
        ...
        <x-card class="p-0 border-none shadow-2xl rounded-[4rem] bg-white overflow-hidden group italic" shadow="none">
                        ...
                        <tbody class="italic font-bold">
                            @foreach($proyectos as $proyecto)
                                <tr class="group/row italic">
                                    <td class="px-8 py-8 bg-slate-50 rounded-l-[2rem] border-none group-hover/row:bg-emerald-50 transition-all duration-300 italic font-bold">
                                        <div class="space-y-2 italic text-slate-900">
                                            <p class="text-sm font-black text-slate-900 uppercase italic tracking-tighter leading-tight group-hover/row:text-emerald-700 transition-colors">
                                            ...
                                            <div class="flex items-center gap-3 text-[9px] font-black text-slate-400 uppercase italic opacity-60 italic">
                                                <i class="far fa-clock text-emerald-500 italic"></i>
                                                <span>LANZADO: {{ \Carbon\Carbon::parse($proyecto->pro_fecha_publi)->translatedFormat('d M, Y') }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-8 py-8 bg-slate-50 border-none group-hover/row:bg-emerald-50 transition-all duration-300 italic">
                                        <x-badge class="bg-white text-slate-500 border-none px-4 py-2 rounded-xl text-[9px] font-black italic shadow-sm uppercase tracking-widest italic group-hover/row:text-emerald-600">
                                            {{ $proyecto->pro_categoria }}
                                        </x-badge>
                                    </td>
                                    <td class="px-8 py-8 bg-slate-50 border-none text-center group-hover/row:bg-emerald-50 transition-all duration-300 italic">
                                        @php
                                    ...
                                    <td class="px-8 py-8 bg-slate-50 border-none text-center group-hover/row:bg-emerald-50 transition-all duration-300 italic">
                                        <div class="inline-flex flex-col items-center italic text-slate-900">
                                            <span class="text-2xl font-black text-slate-900 group-hover/row:text-emerald-600 transition-colors italic tracking-tighter leading-none">{{ $proyecto->pro_duracion_estimada }}</span>
                                            <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest italic opacity-60 italic">DÍAS ESTIMADOS</span>
                                        </div>
                                    </td>
                                    <td class="px-8 py-8 bg-slate-50 rounded-r-[2rem] border-none text-right group-hover/row:bg-emerald-50 transition-all duration-300 italic font-bold">
                                        <div class="flex items-center justify-end gap-3 italic font-bold">
                                            <a href="{{ route('empresa.proyectos.edit', $proyecto->pro_id) }}" 
                                               class="w-12 h-12 rounded-[1.25rem] bg-white border border-slate-100 text-slate-400 hover:text-emerald-500 hover:border-emerald-500 shadow-xl transition-all hover:scale-110 active:scale-95 italic font-bold flex items-center justify-center">
            ...
            @else
                <div class="py-40 text-center italic">
                    ...
                    <div class="mt-12">
                        <x-button variant="primary" :href="route('empresa.proyectos.crear')" shadow="emerald" class="py-6 px-12 rounded-2xl text-[11px] font-black uppercase italic shadow-2xl active:scale-95">
