...
<?php $__env->startSection('page-title', 'Administración de Cuentas'); ?>
    ...
    <div class="animate-fade-in" style="margin-bottom: 40px;">
        ...
        <div style="display:flex; gap:12px; margin:24px 0; position: relative; z-index: 10;">
            <button id="btn-aprendices" onclick="mostrarTabla('aprendices')" style="display:flex;align-items:center;gap:10px;padding:14px 28px;border:none;border-radius:14px;background:linear-gradient(135deg,#2e7d46,#1a5c30);color:white;font-weight:700;font-size:14px;cursor:pointer;box-shadow:0 6px 20px rgba(46,125,70,0.4);transition:all 0.3s; z-index: 11;">
                ...
                <span style="background:rgba(255,255,255,0.25);padding:2px 10px;border-radius:20px;font-size:12px;"><?php echo e($aprendices->count()); ?></span>
            </button>
            <button id="btn-instructores" onclick="mostrarTabla('instructores')" style="display:flex;align-items:center;gap:10px;padding:14px 28px;border:2px solid #2e7d46;color:#2e7d46;border-radius:14px;background:transparent;font-weight:700;font-size:14px;cursor:pointer;transition:all 0.3s; z-index: 11;">
                ...
                <span style="background:#2e7d46;color:white;padding:2px 10px;border-radius:20px;font-size:12px;"><?php echo e($instructores->count()); ?></span>
            </button>
        </div>
                    ...
                    <table style="width:100%;border-collapse:collapse;min-width:700px;">
                        ...
                        <tbody>
                            ...
                            <tr style="transition:background 0.2s;" onmouseover="this.style.background='#f0f5f2'" onmouseout="this.style.background='white'">
                                <td style="padding:18px 20px;font-weight:700;color:#0a3d2a;border-bottom:1px solid #e8f5e9;"><?php echo e($a->usuario->numero_documento ?? '-'); ?></td>
                                <td style="padding:18px 20px;border-bottom:1px solid #e8f5e9;">
                                    <div style="display:flex;align-items:center;gap:12px;">
                                        <div style="width:36px;height:36px;background:linear-gradient(135deg,#2e7d46,#4caf50);border-radius:10px;display:flex;align-items:center;justify-content:center;font-weight:800;color:white;font-size:13px;"><?php echo e(strtoupper(substr($a->nombres,0,1))); ?></div>
                                        <span style="font-weight:600;color:#0a3d2a;"><?php echo e($a->nombres); ?> <?php echo e($a->apellidos); ?></span>
                                    </div>
                                </td>
                                <td style="padding:18px 20px;border-bottom:1px solid #e8f5e9;">
                                    <span style="background:#e8f5e9;color:#1a5c30;padding:6px 14px;border-radius:8px;font-size:12px;font-weight:700;border:1px solid #a5d6a7;"><?php echo e($a->programa_formacion); ?></span>
                                </td>
                                <td style="padding:18px 20px;color:#1a5c30;font-size:13px;border-bottom:1px solid #e8f5e9;"><?php echo e($a->usuario->correo ?? '-'); ?></td>
                                <td style="padding:18px 20px;border-bottom:1px solid #e8f5e9;">
                                    <span style="display:inline-flex;align-items:center;gap:6px;background:<?php echo e($a->activo == 1 ? '#e8f5e9' : '#ffebee'); ?>;color:<?php echo e($a->activo == 1 ? '#1a5c30' : '#c62828'); ?>;padding:6px 14px;border-radius:20px;font-size:11px;font-weight:700;border:1px solid <?php echo e($a->activo == 1 ? '#a5d6a7' : '#ef9a9a'); ?>;">
                                        <span style="width:6px;height:6px;border-radius:50%;background:<?php echo e($a->activo == 1 ? '#2e7d46' : '#e53935'); ?>;"></span>
                                        <?php echo e($a->activo == 1 ? 'aprobado' : 'cerrado'); ?>

                                    </span>
                                </td>
                                <td style="padding:18px 20px;text-align:right;border-bottom:1px solid #e8f5e9;">
                                    <form action="<?php echo e(route('admin.usuarios.estado', $a->id)); ?>" method="POST" style="display:inline;">
                                        <?php echo csrf_field(); ?>
                                        <input type="hidden" name="tipo" value="aprendiz">
                                        <input type="hidden" name="estado" value="<?php echo e($a->activo == 1 ? 0 : 1); ?>">
                                        <button type="submit" style="display:inline-flex;align-items:center;gap:6px;padding:10px 18px;border:<?php echo e($a->activo == 1 ? '2px solid #2e7d46' : 'none'); ?>;background:<?php echo e($a->activo == 1 ? 'transparent' : '#2e7d46'); ?>;color:<?php echo e($a->activo == 1 ? '#2e7d46' : 'white'); ?>;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;">
                                            <i class="fas <?php echo e($a->activo == 1 ? 'fa-user-minus' : 'fa-user-check'); ?>"></i>
                                            <?php echo e($a->activo == 1 ? 'Desactivar' : 'Activar'); ?>
            ...
            <div style="background:white;border-radius:20px;box-shadow:0 8px 30px rgba(46,125,70,0.12);overflow:hidden;border:1px solid rgba(46,125,70,0.1);">
                ...
                <div style="overflow-x:auto;">
                    <table style="width:100%;border-collapse:collapse;min-width:700px;">
                        ...
                        <tbody>
                            <?php $__empty_1 = true; $__currentLoopData = $instructores; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr style="transition:background 0.2s;" onmouseover="this.style.background='#f0f5f2'" onmouseout="this.style.background='white'">
                                <td style="padding:18px 20px;font-weight:700;color:#0a3d2a;border-bottom:1px solid #e8f5e9;"><?php echo e($i->usuario->numero_documento ?? '-'); ?></td>
                                <td style="padding:18px 20px;border-bottom:1px solid #e8f5e9;">
                                    <div style="display:flex;align-items:center;gap:12px;">
                                        <div style="width:36px;height:36px;background:linear-gradient(135deg,#2e7d46,#4caf50);border-radius:10px;display:flex;align-items:center;justify-content:center;font-weight:800;color:white;font-size:13px;"><?php echo e(strtoupper(substr($i->nombres,0,1))); ?></div>
                                        <span style="font-weight:600;color:#0a3d2a;"><?php echo e($i->nombres); ?> <?php echo e($i->apellidos); ?></span>
                                    </div>
                                </td>
                                <td style="padding:18px 20px;border-bottom:1px solid #e8f5e9;">
                                    <span style="background:#e8f5e9;color:#1a5c30;padding:6px 14px;border-radius:8px;font-size:12px;font-weight:700;border:1px solid #a5d6a7;"><?php echo e($i->especialidad); ?></span>
                                </td>
                                <td style="padding:18px 20px;color:#1a5c30;font-size:13px;border-bottom:1px solid #e8f5e9;"><?php echo e($i->usuario->correo ?? '-'); ?></td>
                                <td style="padding:18px 20px;border-bottom:1px solid #e8f5e9;">
                                    <span style="display:inline-flex;align-items:center;gap:6px;background:<?php echo e($i->activo == 1 ? '#e8f5e9' : '#ffebee'); ?>;color:<?php echo e($i->activo == 1 ? '#1a5c30' : '#c62828'); ?>;padding:6px 14px;border-radius:20px;font-size:11px;font-weight:700;border:1px solid <?php echo e($i->activo == 1 ? '#a5d6a7' : '#ef9a9a'); ?>;">
                                        <span style="width:6px;height:6px;border-radius:50%;background:<?php echo e($i->activo == 1 ? '#2e7d46' : '#e53935'); ?>;"></span>
                                        <?php echo e($i->activo == 1 ? 'aprobado' : 'cerrado'); ?>

                                    </span>
                                </td>
                                <td style="padding:18px 20px;text-align:right;border-bottom:1px solid #e8f5e9;">
                                    <form action="<?php echo e(route('admin.usuarios.estado', $i->id)); ?>" method="POST" style="display:inline;">
                                        <?php echo csrf_field(); ?>
                                        <input type="hidden" name="tipo" value="instructor">
                                        <input type="hidden" name="estado" value="<?php echo e($i->activo == 1 ? 0 : 1); ?>">
                                        <button type="submit" style="display:inline-flex;align-items:center;gap:6px;padding:10px 18px;border:<?php echo e($i->activo == 1 ? '2px solid #2e7d46' : 'none'); ?>;background:<?php echo e($i->activo == 1 ? 'transparent' : '#2e7d46'); ?>;color:<?php echo e($i->activo == 1 ? '#2e7d46' : 'white'); ?>;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;">
                                            <i class="fas <?php echo e($i->activo == 1 ? 'fa-user-minus' : 'fa-user-check'); ?>"></i>
                                            <?php echo e($i->activo == 1 ? 'Desactivar' : 'Activar'); ?>
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\Bolsa_Proyecto_Sena\resources\views/admin/usuarios.blade.php ENDPATH**/ ?>