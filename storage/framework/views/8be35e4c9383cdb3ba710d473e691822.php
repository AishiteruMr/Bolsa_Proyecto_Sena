    ...
    <div style="display: flex; flex-direction: column; gap: 2rem;">
        ...
        <div style="background: white; padding: 2rem; border-radius: 20px; border: 1px solid rgba(62,180,137,0.1);">
            ...
            <div style="margin-bottom: 2rem; padding: 14px 18px; background: linear-gradient(135deg, rgba(62,180,137,0.08), rgba(62,180,137,0.04)); border-radius: 12px; border: 1px solid rgba(62,180,137,0.15); display: flex; align-items: center; justify-content: space-between;">
                ...
                <div style="display: flex; gap: 8px;">
                    <a href="<?php echo e(asset('storage/' . $proyecto->url_estructura)); ?>" target="_blank" style="padding: 8px 16px; background: linear-gradient(135deg, #3eb489, #2d9d74); color: white; border-radius: 10px; font-size: 0.8rem; font-weight: 700; text-decoration: none; display: flex; align-items: center; gap: 6px;">
                        <i class="fas fa-eye"></i> Ver
                    </a>
                    <a href="<?php echo e(asset('storage/' . $proyecto->url_estructura)); ?>" download style="padding: 8px 16px; background: rgba(62,180,137,0.1); color: #3eb489; border-radius: 10px; font-size: 0.8rem; font-weight: 700; text-decoration: none; display: flex; align-items: center; gap: 6px;">
            ...
            <div style="display: flex; flex-direction: column; gap: 1rem;">
                <?php $__empty_1 = true; $__currentLoopData = $etapas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $etapa): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="instructor-stage-card" id="stage-card-<?php echo e($etapa->id); ?>">
                        
                        <div class="instructor-stage-view" id="stage-view-<?php echo e($etapa->id); ?>">
                            <div class="instructor-stage-number" style="background: <?php echo e($index == 0 ? 'linear-gradient(135deg, #3eb489, #2d9d74)' : 'rgba(62,180,137,0.08)'); ?>; color: <?php echo e($index == 0 ? 'white' : '#3eb489'); ?>;">
                                <?php echo e($etapa->orden); ?>

                            </div>
                            <div style="flex: 1;">
                                <h4 style="font-weight: 800; color: var(--text); margin-bottom: 6px; font-size: 1.1rem;"><?php echo e($etapa->nombre); ?></h4>
                                <p style="font-size: 0.9rem; color: var(--text-light); line-height: 1.6; font-weight: 500;"><?php echo e($etapa->descripcion); ?></p>
                                ...
                                <div style="margin-top: 10px; padding: 10px 14px; background: rgba(62,180,137,0.08); border-radius: 10px; display: inline-flex; align-items: center; gap: 8px;">
                                    ...
                                    <a href="<?php echo e(asset('storage/' . $etapa->url_documento)); ?>" target="_blank" style="color: #3eb489; font-size: 0.85rem; font-weight: 700; text-decoration: none;">
                                        Ver documento/guide
                                    </a>
                                </div>
                                <?php endif; ?>
                            ...
                            <div style="display: flex; gap: 8px;">
                                <button type="button" onclick="document.getElementById('docForm-<?php echo e($etapa->id); ?>').classList.toggle('active')" style="width: 36px; height: 36px; border-radius: 10px; background: rgba(62,180,137,0.1); color: #3eb489; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center;" title="Subir documento">
                                    <i class="fas fa-upload"></i>
                                </button>
                                <button type="button" onclick="toggleEditStage(<?php echo e($etapa->id); ?>)" style="width: 36px; height: 36px; border-radius: 10px; background: rgba(62,180,137,0.1); color: #3eb489; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s;" title="Editar etapa">
                                    <i class="fas fa-pen"></i>
                                </button>
                                <form action="<?php echo e(route('instructor.etapas.eliminar', $etapa->id)); ?>" method="POST" id="eliminar-etapa-<?php echo e($etapa->id); ?>">
                                    <?php echo csrf_field(); ?>
                                    <?php echo method_field('DELETE'); ?>
                                    <button type="button" data-confirm="¿Eliminar etapa?" data-confirm-title="Eliminar Etapa" onclick="openConfirm('¿Eliminar etapa?', 'La etapa &quot;<?php echo e(Str::limit($etapa->nombre, 25)); ?>&quot; será eliminada permanentemente.', () => document.getElementById('eliminar-etapa-<?php echo e($etapa->id); ?>').submit())" style="width: 36px; height: 36px; border-radius: 10px; background: rgba(239, 68, 68, 0.1); color: #ef4444; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center;" class="btn-del-hover">
                                        <i class="fas fa-trash-alt"></i>
                                    </button>
                                </form>
                            </div>
                        ...
                        <div id="stage-edit-<?php echo e($etapa->id); ?>" style="display: none; width: 100%;">
                            <form action="<?php echo e(route('instructor.etapas.editar', $etapa->id)); ?>" method="POST">
                                ...
                                <div style="display: grid; grid-template-columns: 80px 1fr; gap: 1rem; margin-bottom: 1rem;">
                                    <input type="number" name="orden" value="<?php echo e($etapa->orden); ?>" placeholder="N°" required class="aprendiz-input-control">
                                    <input type="text" name="nombre" value="<?php echo e($etapa->nombre); ?>" placeholder="Título de la etapa..." required class="aprendiz-input-control">
                                </div>
                                <textarea name="descripcion" placeholder="Descripción de la etapa..." required class="aprendiz-input-control" style="min-height: 80px; margin-bottom: 1rem;"><?php echo e($etapa->descripcion); ?></textarea>
                                <div style="display: flex; justify-content: flex-end; gap: 1rem;">
                                    <button type="button" onclick="toggleEditStage(<?php echo e($etapa->id); ?>)" style="background: transparent; border: none; font-weight: 700; color: var(--text-light); cursor: pointer; padding: 8px 16px;">Cancelar</button>
                                    <button type="submit" class="btn-premium" style="width: auto; padding: 10px 24px;">
                        ...
                        <div id="docForm-<?php echo e($etapa->id); ?>" class="instructor-collapsible" style="display: none; margin-top: 1rem;">
